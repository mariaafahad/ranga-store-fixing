...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${product.image}></img>
      ...
      <h5>Raiting: ${product.rating.rate}</h5>
      <h5>Person rated: ${product.rating.count}</h5>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-info" style="border:none">add to cart</button>
      <button id="details-btn" class="btn btn-warning">Details</button></div>
      `;
    document.getElementById("all-products").appendChild(div);
  }
};
// above inside innerHtml tag  image+title+catagory+price=rate+pplraiting is been dynamically set
// and also in button product id and its price is set so that it can add it values
let count = 0;
...
const getInputValue = (id) => {
  ...
  // parseFloat for point value addition
  return converted;
};
...
const updatePrice = (id, value) => {
  ...
  // done to.Fixed for getting up to 2 digit after point
};
...
// above some condition in if set for calculating delivary charge + tax charge
